...
export class RemoveUser {
  ...
  process( splitedMessage, message ) {
    if ( !splitedMessage[1] ) 
      return message.reply('Valid command usage: !remove <username> ( type "_" for space )')

    const userToRemove = splitedMessage[1].replace( /_/g, ' ');
    const user = message.guild.members.cache.find( member => member.user.username == userToRemove );
    
    if ( !user ) 
      return message.reply(`User ${userToRemove} not found`);
    try {
      const roleToRemove = message.member.guild.roles.cache.find( role => role.name == 'streamer approved' );
